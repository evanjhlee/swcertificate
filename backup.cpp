...
const  int NM = 500 + 5;
const  int KM = 4'000 + 3'000 + 5;

struct Node {
	int id, to, time, power;
};
vector<Node> a[NM];
unordered_map<int, int> id2start;
int N, K, charge[NM];
void init(int N, int mCharge[], int K, int mId[], int sCity[], int eCity[], int mTime[], int mPower[]) {
	...
	ff(i, 0, K) {
		id2start[mId[i]] = sCity[i];
		a[sCity[i]].push_back({ mId[i], eCity[i], mTime[i], mPower[i] });
	}
}
void add(int mId, int sCity, int eCity, int mTime, int mPower) {
	id2start[mId] = sCity;
	a[sCity].push_back({ mId, eCity, mTime, mPower });
}
void remove(int mId) {
	auto it = id2start.find(mId);
	if (it == id2start.end()) return;        // 없는 ID면 무시
	...
	id2start.erase(it);                       // 매핑도 정리
}

int vDist[NM], dist[NM][305];
int cost(int B, int sCity, int eCity, int M, int mCity[], int mTime[]) {
	memset(vDist, 0x7f, sizeof(vDist));
	priority_queue<ull, vector<ull>, greater<ull>> vpq;
	ff(i, 0, M) {
		vDist[mCity[i]] = mTime[i];
		vpq.push((ull)mTime[i] << 32 | (ull)mCity[i]);
	}
	while (!vpq.empty()) {
		int u = vpq.top() & 0xFFFFFFFF;
		int utime = vpq.top() >> 32;
		vpq.pop();

		if (utime > vDist[u]) continue;

		for (auto& i : a[u]) {
			int v = i.to;
			int vtime = i.time;

			int ntime = utime + vtime;

			if (vDist[v] > ntime) {
				vDist[v] = ntime;
				vpq.push((ull)ntime << 32 | (ull)v);
			}
		}
	}
	...
	while (!pq.empty()) {
		int utime = pq.top() >> 32;
		int u = (pq.top() >> 16) & 0xFFFF;
		int ubattery = pq.top() & 0xFFFF;
		pq.pop();


		// 감염된(또는 동시) 시각의 상태는 폐기
		if (utime >= vDist[u]) continue;
		...
		for (auto& i : a[u]) {
			...
			int vpower = i.power;
			int ntime = utime + vtime;
			int nbattery = ubattery - vpower;

			if (ntime >= vDist[v]) continue;
			if (nbattery < 0) continue;

			if (dist[v][nbattery] > ntime) {
				for (rint i = nbattery; i >= 0; i--) {
					if (dist[v][i] < ntime) break;
					dist[v][i] = ntime;
				}
				pq.emplace((ull)ntime << 32 | (ull)v << 16 | (ull)nbattery);
			}
		}
		...
		if (ubattery < B) {
			int ntime = utime + 1;
			int nbattery = min(B, ubattery + charge[u]);

			if (ntime >= vDist[u]) continue;

			if (dist[u][nbattery] > ntime) {
				for (rint i = nbattery; i >= 0; i--) {
					if (dist[u][i] < ntime) break;
					dist[u][i] = ntime;
				}
				pq.emplace((ull)ntime << 32 | (ull)u << 16 | (ull)nbattery);
			}
		}
	}
	log("%8s %d\n", __func__, -1);
...
/*
2. 각 테스트 케이스에서 add() 함수의 호출 횟수는 3,000 이하이다.
3. 각 테스트 케이스에서 remove() 함수의 호출 횟수는 100 이하이다.
4. 각 테스트 케이스에서 call() 함수의 호출 횟수는 10,000 이하이다.
5. 각 테스트 케이스에서 search() 함수의 호출 횟수는 40,000 이하이다.
6. 각 테스트 케이스에서 add()와 call()로 전달되는 서로 다른 전화번호의 개수는 5,000 이하이다.
*/
const int NM = 100'000 + 5;
const int TM = (23'000 * 8) + 5;
const int DM = (25'000 * 8) + 5;
#define MAX_N			(5)
#define MAX_L			(8)
...
struct Trie {
	...
	bool addWord(const char* s) {
		Node* u = root;
		u->sub++;
		for (rint i = 0; s[i]; ++i) {
			if (!u->child.count(s[i]))
				u->child[s[i]] = newNode();
			u = u->child[s[i]];
			u->sub++;
		}
	...
	bool contain(const char* s) {
		...
		return u->end > 0 ? true : false;
	}
	bool removeWord(const char* s) {
		assert(contain(s));
		Node* u = root;
		u->sub--;
		for (rint i = 0; s[i]; ++i) {
	...
	int getRank(const char* s) {
		...
		return u->end > 0 ? rank + 1 : -1;
	}
	bool getKthWord(int k, string& s) {
		if (root->sub < k)
			return false;
		k--;
		Node* u = root;
	...
	void dfs(Node* u, string& s) {
		...
		for (auto& i : u->child) {
			log("%c [%d:%d]\n", i.first, i.second->sub, i.second->end);
			s.push_back(i.first);
			dfs(i.second, s);
			s.pop_back();
		}
...
int main() {
	...
	for (int i = 1; i <= trie.root->sub; ++i) {
		string s;
		if (trie.getKthWord(i, s)) {
			log("%d: %s\n", i, s.c_str());
		}
	}
...
vector<vector<int>> weights; // [M][5]
vector<priority_queue<pair<int, int>>> universityPQ; // [M]
vector<int> assignedCount; // [M]

void init(int n, int m, int weight[30][5]) {
	N = n;
	M = m;
	nextID = 1;
	students.clear();
	weights.assign(M, vector<int>(5));
	universityPQ.assign(M, priority_queue<pair<int, int>>());
	assignedCount.assign(M, 0);
...
void add(int score[5]) {
	...
	for (int i = 0; i < M; ++i) {
		int total = 0;
		for (int j = 0; j < 5; ++j)
			total += score[j] * weights[i][j];
		universityPQ[i].emplace(total, -s.id); // -id for tie-breaker
	}
}

void eraser(int id) {
	if (id >= 1 && id <= (int)students.size())
		students[id - 1].valid = false;
}

int suggestion(int mID) {
	if (assignedCount[mID] >= N) return -1;

	auto& pq = universityPQ[mID];
	while (!pq.empty()) {
		int id = -pq.top().second;
		pq.pop();
		if (id < 1 || id >(int)students.size()) continue;
		Student& s = students[id - 1];
		if (!s.valid || s.assigned) continue;
...
void add(int id, int score[5]) {
	...
	for (int i = 0; i < M; ++i) {
		...
		for (int j = 0; j < 5; ++j)
			total += score[j] * weights[i][j];
		universityPQ[i].emplace(total, -id); // -id for tie-breaker
	}
}

void eraser(int id) {
	if (studentMap.count(id))
		studentMap[id].valid = false;
}
...
// 위쪽 경계 처리
for (int i = x1; i < (x1 + BSX - x1 % BSX) && i <= x2; ++i) {
	for (int j = y1; j <= y2; ++j) {
...
// 가운데 블록 처리
for (int i = (x1 + BSX - x1 % BSX); i + BSX - 1 <= x2; i += BSX) {
	for (int j = (y1 + BSY - y1 % BSY); j + BSY - 1 <= y2; j += BSY) {
		maxValue = std::max(maxValue, b[i / BSX][j / BSY].maxv);
		minValue = std::min(minValue, b[i / BSX][j / BSY].minv);
	}
}

// 오른쪽 경계 처리
for (int i = x2 - x2 % BSX + 1; i <= x2; ++i) {
	for (int j = y1; j <= y2; ++j) {
		maxValue = std::max(maxValue, a[i][j]);
		minValue = std::min(minValue, a[i][j]);
	}
...
// 좌우 경계 처리 (위쪽과 아래쪽 경계 안에 포함됨)
for (int i = (x1 + BSX - x1 % BSX); i + BSX - 1 <= x2; i += BSX) {
	for (int j = y1; j < (y1 + BSY - y1 % BSY) && j <= y2; ++j) {
		maxValue = std::max(maxValue, a[i][j]);
		minValue = std::min(minValue, a[i][j]);
	}
	for (int j = y2 - y2 % BSY + 1; j <= y2; ++j) {
		maxValue = std::max(maxValue, a[i][j]);
		minValue = std::min(minValue, a[i][j]);
	}
}
...
for (int i = x1; i <= x2; ) {
	if (i % BSX == 0 && i + BSX - 1 <= x2) {
		for (int j = y1; j <= y2; ) {
			if (j % BSY == 0 && j + BSY - 1 <= y2) {
				maxValue = std::max(maxValue, b[i / BSX][j / BSY].maxv);
				minValue = std::min(minValue, b[i / BSX][j / BSY].minv);
				j += BSY;
			}
...
for (int i = x1; i <= x2; ) {
	for (int j = y1; j <= y2; ) {
		if (i % bs == 0 && j % bs == 0 && i + bs - 1 <= x2 && j + bs - 1 <= y2) {
			maxValue = std::max(maxValue, b[i / bs][j / bs].maxv);
			minValue = std::min(minValue, b[i / bs][j / bs].minv);
			j += bs;
		} 
		else {
			maxValue = std::max(maxValue, a[i][j]);
...
struct Trie {
	...
	bool getKthWord(int k, string& ans) {
		return false;
	}
	int getRank(const char* s) {
		Node* u = root;
		...
		return u->end == 0 ? -1 : cnt + 1;
	}


	int getPrefixCnt(const char* s) {
		return 1;
	}
	...
	void dfs(Node* u) {
		...
		for (auto& v : u->child) {
			log("%c [ %d : %d ]\n", v.first, v.second->sub, v.second->end);
			dfs(v.second);
		}
	}
...
struct Trie {
	...
	bool contain(const char* s) {
		...
		for (rint i = 0; s[i]; ++i) {
			int d = s[i] - 'a';
			if (nodepool[u].child[d] == -1)
				return 0;
			u = nodepool[u].child[d];
		}
		return nodepool[u].end > 0;
	}
	bool addWord(const char* s) {
		int u = root;
		nodepool[u].sub++;
	...
	bool getKthWord(int k, string& ans) {
		...
		ans.clear();
		while (1) {
			if (k <= 0 || k > nodepool[root].sub) return false;
			return true;

			k -= nodepool[u].end;

			for (int i = 0; i < 26; ++i) {
				int v = nodepool[u].child[i];
				if (v == -1) continue;
				if (k <= 0 || k <= nodepool[v].sub) {
					ans.push_back(i + 'a');
					u = v;
					break;
				}
...
bool search(const char s[]) {
	...
	return node[u].end > 0 ? true : false;
}
int add(char s[])
{
	if (search(s)) {
...
myRESULT ret;
myRESULT find(char mInitial, int mIndex)
{
	...
	int u = node[root].child[mInitial - 'a'];
	ret.word[len++] = mInitial;
	...
	while (1) {
		if (node[u].end >= mIndex) {
			ret.word[len++] = '\0';
			ret.success = 1;
			log("%-8s(%8c, %d)\t\tret=(%2d, %s)\n", __func__, mInitial, mIndex, 1, ret.word);
			return ret;
		}
		mIndex -= node[u].end;
	...
	log("%-8s(%8c, %d)\t\tret=%s %2d\n", __func__, mInitial, mIndex, ret.word, 0);
...
void dfs(int node) {
    visited[node] = true;   // ★ 진입하자마자 방문 처리
...
void bfs(int start) {
    queue<int> q;
    visited[start] = true;  // ★ 큐에 넣기 전에 방문 처리
    q.push(start);
    ...
    while (!q.empty()) {
        ...
        for (int next : adj[node]) {
            if (!visited[next]) {
                visited[next] = true;  // ★ push 직전에 방문 처리
                q.push(next);
            }
        }
    }
...
void backtracking(int depth) {
    ...
    for (int i = 1; i <= N; i++) {
        if (!visited[i]) {
            visited[i] = true;    // ★ 선택 직전에 방문 처리
            arr[depth] = i;
            backtracking(depth + 1);
            visited[i] = false;   // ★ 재귀 끝나면 원상복구
        }
    }
}


위치: 선택 직전, 그리고 재귀 끝나면 원상복구

특징: true → 재귀 → 다시 false (✅ 되돌림 필요)

목적: 경우의 수를 모두 탐색하기 위함



📌 기억하기 좋은 문장

DFS: "들어가자마자 방문 처리" (영구적)

BFS: "큐에 넣기 전에 방문 처리" (영구적)

백트래킹: "선택할 때 방문, 되돌아올 때 해제" (임시적)
...
typedef pair<int, int> pii;
/*
2. 각 테스트 케이스에서 add() 함수의 호출 횟수는 2000 이하이다.
3. 각 테스트 케이스에서 calculate() 함수의 호출 횟수는 100 이하이다.
*/

const int NM = 10'000 + 5;
const int KM = 30'000 + 2'000 + 5;

vector<pii> a[NM];
int N;
void add(int sBuilding, int eBuilding, int mDistance) {
	a[sBuilding].push_back({ eBuilding, mDistance });
	a[eBuilding].push_back({ sBuilding, mDistance });
}
...
int R;
int cb[NM];
void dijkstra(int dist[], int M, int b[]) {
	
	priority_queue<pii, vector<pii>, greater<pii>> pq;	// distance, to 
	ff(i, 0, M) {
		dist[b[i]] = 0;
		pq.push({ 0, b[i] });
		cb[b[i]] = 1;
	...
	while (!pq.empty()){
		...
		for (auto& i : a[u]) {
			...
			if (w > R) continue;
			int v = i.second;
			int ndistance = dist[u] + w;
			if (ndistance > R) continue;
...
int dist_cof[NM], dist_bak[NM];
int calculate(int M, int mCoffee[], int P, int mBakery[], int R) {
	::R = R;
	ff(i, 0, N) {
		dist_cof[i] = dist_bak[i] = INF;
		cb[i] = 0;
	}
	dijkstra(dist_cof, M, mCoffee);
	dijkstra(dist_bak, P, mBakery);

	int minv = INF;
	ff(i, 0, N) {
		if (cb[i]) continue;
		int d = dist_cof[i] + dist_bak[i];
		if (d > 2 * R) continue;
		if (minv > d) {
			minv = d;
		}
	}
	int ret = minv == INF ? -1 : minv;
	log("%8s\t\tret=%2d\n", __func__, ret);
	return ret;
}
...
const int MAX_REQ = 50000;
const int MAX_ROB = 1000;

struct Request {
	int id, grade, time, workload, original_grade;
	bool assigned = false;
	int start_time = -1;
} request_pool[MAX_REQ];

struct Robot {
	int id, throughput, available_time = 0;
} robot_pool[MAX_ROB];

int reqCnt = 0, robCnt = 0;
int escalation_delay;

vector<pair<int, int>> completed; // {request_index, end_time}

struct RequestCmp {
	bool operator()(const int &a, const int &b) const {
		return 
			request_pool[a].grade != request_pool[b].grade ? 
			request_pool[a].grade < request_pool[b].grade :
			request_pool[a].time > request_pool[b].time;
	}
};
...
struct RobotCmp {
	bool operator()(const int& a, const int& b) const {
		return
			robot_pool[a].throughput != robot_pool[b].throughput ?
			robot_pool[a].throughput < robot_pool[b].throughput :
			robot_pool[a].id > robot_pool[b].id;
	}
};

// 이벤트 큐: {available_time, robot_index}
priority_queue<pair<int, int>, vector<pair<int, int>>, greater<>> event_queue;
priority_queue<int, vector<int>, RequestCmp> pending_requests;
priority_queue<int, vector<int>, RobotCmp> available_robots;

void add_robot(int time, int id, int throughput) {
	robot_pool[robCnt] = { id, throughput, time };
	event_queue.push({ time, robCnt });
	robCnt++;
}

void add_request(int id, int time, int workload, int grade) {
	request_pool[reqCnt] = { id, grade, time, workload, grade };
	pending_requests.push(reqCnt++);
}

void process_events(int current_time) {
	while (!event_queue.empty() && event_queue.top().first <= current_time) {
		int rob_idx = event_queue.top().second;
		event_queue.pop();
		available_robots.push(rob_idx);
	}
}

void assign_requests(int current_time) {
	process_events(current_time); // 먼저 이벤트 처리

	while (!pending_requests.empty() && !available_robots.empty()) {
		int req_idx = pending_requests.top(); pending_requests.pop();
		int rob_idx = available_robots.top(); available_robots.pop();

		Request& req = request_pool[req_idx];
		Robot& rob = robot_pool[rob_idx];

		req.assigned = true;
		req.start_time = current_time;
		int duration = ceil((double)req.workload / rob.throughput);
		rob.available_time = current_time + duration;

		completed.push_back({ req_idx, rob.available_time });
		event_queue.push({ rob.available_time, rob_idx }); // 작업 완료 이벤트 등록
	}
}

int escalate(int time, int mid) {
	vector<int> temp;
	while (!pending_requests.empty()) {
		int idx = pending_requests.top(); pending_requests.pop();
		Request& req = request_pool[idx];
		if (!req.assigned && time - req.time >= escalation_delay && req.grade < 10) {
			req.grade = 10;
		}
		temp.push_back(idx);
	}
	for (int idx : temp) pending_requests.push(idx);

	int total_delay = 0;
	for (auto& p : completed) {
		Request& req = request_pool[p.first];
		if (req.original_grade < 10 && req.grade == 10 && p.second >= time) {
			total_delay += req.start_time - req.time;
		}
	}
	return total_delay;
}
...
struct RequestCmp {
	bool operator()(int a, int b) const {
		const Request& ra = request_pool[a];
		const Request& rb = request_pool[b];
...
struct RobotCmp {
	bool operator()(int a, int b) const {
		const Robot& ra = robot_pool[a];
		const Robot& rb = robot_pool[b];
		if (ra.throughput != rb.throughput) return ra.throughput < rb.throughput;
		return ra.id > rb.id;
	}
};
...
void assign_requests(int current_time) {
	while (!pending_requests.empty() && !available_robots.empty()) {
		...
		if (rob.available_time > current_time) {
			pending_requests.push(req_idx);
			available_robots.push(rob_idx);
			break;
		}
...
bool searchWord(const char* s) {
	...
	for (rint i = 0; s[i]; ++i) {
		int d = s[i] - 'a';
		if (node[u].child[d] == -1)	return false;
		u = node[u].child[d];
	}
	return node[u].end > 0;
}
...
int main() {
	...
	out.clear();
	kthWord(getRank("bath"), out);
	log("%s\n", out.c_str());

	out.clear();
	kthWord(getRank("band"), out);
	log("%s\n", out.c_str());
}
